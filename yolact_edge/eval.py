...
# color_cache = {n: torch.rand(3) for n in range(1)}
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45):
    ...
    # num_dets_to_consider = how many objects
...
def evalvideo(net:Yolact, path:str):
    ...
    def prep_frame(inp):
        with torch.no_grad():
            frame, preds = inp
            return prep_display(preds, frame, None, None, undo_transform=False, class_color=False)
    ...
    def play_video():
        nonlocal frame_buffer, running, video_fps, is_webcam, is_rtsp
        ...
        while running:
            ...
            new_target = frame_time_stabilizer if is_webcam or is_rtsp else max(frame_time_stabilizer, frame_time_target)
